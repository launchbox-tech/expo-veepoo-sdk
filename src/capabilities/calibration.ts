...
function validateBloodPressureCalibration(systolic: number, diastolic: number): void {
  requireInRange(systolic, "systolic", 60, 250);
  requireInRange(diastolic, "diastolic", 60, 250);
}

function validateBloodGlucoseCalibration(value: number): void {
  requireInRange(value, "value", 2, 30);
}

function validateBloodGlucoseRiskLevel(config: BloodGlucoseRiskConfig): void {
