import type { RecoveringInvoke, ThrowingInvoke } from "../../bridge/native-invoke-pipeline";
import type { LogLevel, LogScope, VeepooErrorCode, VeepooEvent, VeepooEventPayload } from "../../types/index";
/**
...
export type DeviceScopedEvent = {
    [K in VeepooEvent]: VeepooEventPayload[K] extends {
        device_id: string;
    } ? K : never;
}[VeepooEvent];
export interface CapabilityContext<TNative> {
    ...
    log: (level: LogLevel, scope: LogScope, action: string, message: string, options?: {
        deviceId?: string;
        data?: unknown;
        error?: unknown;
    }) => void;
