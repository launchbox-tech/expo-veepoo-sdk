...
exports.verifyNativeRejectionContract = verifyNativeRejectionContract;
const native_rejection_mapping_1 = require("../errors/native-rejection-mapping");
const extract_native_reject_codes_1 = require("./extract-native-reject-codes");
/**
 ...
 */
function verifyNativeRejectionContract(repoRoot) {
    const errors = [];
    const extracted = (0, extract_native_reject_codes_1.extractNativeRejectCodes)(repoRoot);
    const allowed = new Set(native_rejection_mapping_1.ALLOWED_NATIVE_REJECT_CODES);
    if (extracted.size !== allowed.size) {
        errors.push(`Native reject code count: extracted ${extracted.size} vs allowlist ${allowed.size}`);
    }
    for (const c of extracted) {
        if (!allowed.has(c)) {
            errors.push(`Native sources emit "${c}" but it is not in ALLOWED_NATIVE_REJECT_CODES — add to src/errors/native-rejection-mapping.ts`);
        }
    }
    for (const c of allowed) {
        if (!extracted.has(c)) {
            errors.push(`ALLOWED_NATIVE_REJECT_CODES includes "${c}" but no .reject("…") found — remove stale entry or restore native call`);
        }
    }
    return errors;
}
//# sourceMappingURL=verify-native-rejection-contract.js.map