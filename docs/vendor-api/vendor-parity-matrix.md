...
| Stored temperature history | `readStoredTemperatureData(date?)`; `storedTemperatureData` (per record) | Shipped — temperature + bodyTemperature (skin); gate: `isSupportReadTempture()` (Android) / `temperatureType > 0` (iOS); iOS: DB query `veepooSDKGetDeviceTemperatureDataWithDate`; Android: `readTemptureDataBySetting` | TBD |
| Stored blood glucose history | `readStoredBloodGlucoseData(date?)`; `storedBloodGlucoseData` (per record) | Shipped — bloodGlucose + level (low/normal/high); gate: `isSupportBloodGlucose()` (Android) / `bloodGlucoseType > 0` (iOS); iOS: `veepooSDKGetDeviceBloodGlucoseDataWithDate`; Android: `readDeviceManualData(BLOOD_GLUCOSE)` | TBD |
| Stored HRV history | `readStoredHrvData(date?)`; `storedHrvData` (per record) | Shipped — hrv value + rrIntervals array; gate: `isSupportHRV()` (Android) / `hrvType != 0` (iOS); iOS: `veepooSDKGetDeviceHrvDataWithDate`; Android: `readHRVOrigin` | TBD |
| Stored ECG history | `readStoredEcgData(date?)`; `storedEcgData` (per record) | Shipped — aveHeart/Hrv/ResRate/QT, duration, filterSignals; gate: `isSupportECG()` (Android) / `ecgType > 0` (iOS); iOS: `veepooSDKGetDeviceOffStoreECGWithDate`; Android: `readECGId` (EEcgDataType.ALL) → `readECGManuallyData` | TBD |
| Stored body composition history | `readStoredBodyCompositionData(date?)`; `storedBodyCompositionData` (per record) | Shipped — BMI, 13 body metrics; gate: `isSupportBodyComponent()` (Android) / `bodyCompositionType > 0` (iOS); iOS: `veepooSDKGetDeviceOffStoreBodyCompositionWithDate`; Android: `readBodyComponentId` → `readBodyComponentData` | TBD |
| PTT (Pulse Transit Time) electrode test | `startPttTest`, `stopPttTest`; `pttTestResult`, `pttStateChanged` | Shipped — **iOS only** (no Android vendor API); heartRate, hrv, qtInterval, signalQuality; `pttStateChanged` registered at Session start via `veepooSDKAddPTTStateListener`; gate: `ecgType > 0` (iOS) | TBD |
