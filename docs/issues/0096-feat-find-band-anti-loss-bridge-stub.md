# 96 — feat(device): Find Band / anti-loss — bridge stub (#95)

> GitHub: https://github.com/launchbox-tech/expo-veepoo-sdk/issues/96
...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Anti-loss / find device: expose vendor find-Band or anti-loss flows so the host app can trigger locate-from-phone when the Band reports the capability via `readDeviceFunctions()`.**. Related `DeviceFunctions` / capability hints (from checklist): **findDeviceByPhoneFunction**.
