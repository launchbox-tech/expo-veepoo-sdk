...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Bluetooth on/off: vendor controls for Bluetooth power if exposed; expect **Android-last** sequencing per CONTEXT; unified JS API with `CAPABILITY_UNSUPPORTED` on Apple if no equivalent.**. Related `DeviceFunctions` / capability hints (from checklist): **(Android-oriented)**.
