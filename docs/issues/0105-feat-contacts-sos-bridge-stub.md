...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Contacts with SOS: manage contact list / SOS behavior per vendor limits (message length, count, etc.).**. Related `DeviceFunctions` / capability hints (from checklist): **contactFunction, contactType, contactMsgLength**.
