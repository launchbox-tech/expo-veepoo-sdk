# 99 — feat(settings): Wrist-flip wake — bridge stub (#95)

> GitHub: https://github.com/launchbox-tech/expo-veepoo-sdk/issues/99
> Status: open | Labels: (update when #99 closed on GitHub)
...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Wrist-flip wake: read/write night wrist-turn / raise-to-wake style settings per vendor API.**. Related `DeviceFunctions` / capability hints (from checklist): **nightTurnSetting, isOpenNightTurnWrist**.
...
- `readWristFlipWakeSettings()` → `WristFlipWakeSettings`
- `setWristFlipWakeSettings(settings)` — validates `sensitivityLevel` 1–10
