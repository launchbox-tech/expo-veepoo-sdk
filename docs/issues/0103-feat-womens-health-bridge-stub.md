...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Women's health: settings or data paths the vendor exposes under the `woman` capability flag.**. Related `DeviceFunctions` / capability hints (from checklist): **woman**.
