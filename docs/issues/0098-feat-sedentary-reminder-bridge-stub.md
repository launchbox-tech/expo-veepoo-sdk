...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Sedentary reminder: read/write long-sit / sedentary alert settings (`LongSeatSetting` or vendor equivalent).**. Related `DeviceFunctions` / capability hints (from checklist): **sedentaryRemind**.
