...
**Stub issue** — vertical slice to be detailed after triage (`docs/checklist.md` §7 Group C/D). Target capability area: **Music / camera remote: HID-style controls from Band when vendor exposes related flags.**. Related `DeviceFunctions` / capability hints (from checklist): **musicStyle, HID-related flags**.
